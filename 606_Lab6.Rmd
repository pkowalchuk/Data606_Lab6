...
Each row in table 6 corresponds to data for each country. That is data for all proportions and the number of samples n.    
...
Table shows this proportion to be 5%, sa as calculated before from the dataset
...
print(paste("Proportion of US 2012 census atheist responces is ",toString(round(nrow(subset(us12,response=="atheist"))/nrow(us12)*100,0)),"%",sep=""))
```
...
Marging of error = z*SE
From above we know SE=0.0069
...
res<-inference(ukr12$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")
```
Using the results
```{r}
SE<-0.0053
...
inference(gr12$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")
```
Using the results
```{r}
SE<-0.0159
...
The relationship between p and me is non linear, but also it has a max at a proportion of 0.5 an minimuns zero at p=0 and p=1   
We can confirm 
this looking at the formula for me

Calculating for the max
```{r}
p<-0.5
2 * sqrt(p * (1 - p)/n)
max(me) # we confirm this is the maximum
```
...
p<-1
2 * sqrt(p * (1 - p)/n)
min(me) # we confirm these are the minimum
```
...
The distributions looks to be normal. As calculated below it is centered on 0.099 ~ 0.1 and has a standard deviation, or indication of spread, of 0.0092   
...
The simulations below show that n affects the spread of the distributions. With larger n we see less spread, or smaller distributions standard deviations.   
...
par(mfrow=c(2,2))
proportionSimulation(1040,0.1)
proportionSimulation(400,0.1)
proportionSimulation(1040,0.02)
proportionSimulation(400,0.02)
```
...
p1<-min(which(me>0.01)) #conpute n for the lower side of the graph
p[p1]
n<-p[p1]*(1-p[p1])/SE^2
ceiling(n)
